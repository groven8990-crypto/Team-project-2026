...
const HELP = {
  ...
  minutes: {
    ...
    items: [
      ...
      "  · 엑셀·표에서 복사한 액션아이템을 안건 칸(또는 '📋 텍스트로 붙여넣기' 박스)에 붙여넣으면 자동으로 여러 행으로 만들어져요. 맨 앞 'No.' 번호열과 머리글(내용/담당자/기한) 줄은 알아서 걸러내요. 기한은 '오늘·내일·금주' 같은 말도 날짜로 바꿔줘요.",
  ...
  reports: {
    ...
    items: [
      ...
      "  · 휴무 표시: 연차·오전반차·오후반차는 구분명 그대로, 외근·출장·교육 등은 일정 제목(예: '수입엑스포 방문')으로 표시돼요.",
...
function leaveLabelFor(memberId, dateStr) {
  ...
  const labels = Store.list("events")
    ...
    .map((e) => {
      // 연차/반차는 구분명 그대로, 그 외(외근·출장·교육 등)는 일정 제목만 표시
      const type = eventLeave(e);
      const title = (e.title || "").trim();
      const PURE = ["연차", "오전반차", "오후반차"];
      if (PURE.includes(type)) return type;
      return title || type;
    });
